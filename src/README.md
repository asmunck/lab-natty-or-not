...
1. Clone o repositório:
   ```bash
1. Extração de texto dos PDFs (src/data/pdfs/).
2. Chunking: divisão do texto em partes menores com referência ao arquivo (src/rag/).
3. Geração de embeddings locais com Sentence Transformers (src/rag/).
4. Indexação dos embeddings em um banco vetorial local (src/data/chroma_db/).
5. LLM local (modelo GGUF, ex: Mistral-7B-Instruct, src/data/models/) responde perguntas usando apenas o contexto recuperado dos PDFs.
6. Interface web para interação (src/app.py).
   git clone https://github.com/seu-usuario/personal-agent.git
   cd personal-agent
   ```
- Dependências listadas em `src/requirements.txt`
